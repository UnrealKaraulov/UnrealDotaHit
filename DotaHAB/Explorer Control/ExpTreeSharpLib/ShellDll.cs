...
namespace ExpTreeLib
{
	public class ShellDll
	{
		...
		//{
			...
			//Danat: private static SHFILEINFO shfitmp; //just used for the following
			...
			//End Structure
			[StructLayout(LayoutKind.Explicit)]public struct STRRET
			{
				[FieldOffset(0)]public int uType; // One of the STRRET_* values
				[FieldOffset(4)]public int pOleStr; // must be freed by caller of GetDisplayNameOf
				[FieldOffset(4)]public int uOffset; // Offset into SHITEMID
				[FieldOffset(4)]public int pStr; // NOT USED
			}
			#endregion
			
			#region "       W32_FIND_DATA"
			[StructLayoutAttribute(LayoutKind.Sequential, CharSet = CharSet.Auto)]public struct WIN32_FIND_DATA
			{
			...
			///</Summary>
			[DllImport("shell32",EntryPoint="SHGetNewLinkInfoA", CharSet=CharSet.Ansi, SetLastError=true)]
			public static extern int SHGetNewLinkInfo(string pszLinkTo, string pszDir, [ Out(), MarshalAs(UnmanagedType.LPStr) ]StringBuilder pszName, ref bool pfMustCopy, SHGNLI uFlags);
			///<Summary> Same function using a PIDL as the pszLinkTo.
			///  SHGNLI.PIDL must be set.
			///</Summary>
			[DllImport("shell32",EntryPoint="SHGetNewLinkInfoA", CharSet=CharSet.Ansi, SetLastError=true)]
			public static extern int SHGetNewLinkInfo(IntPtr pszLinkTo, string pszDir, [ Out(), MarshalAs(UnmanagedType.LPStr) ]StringBuilder pszName, ref bool pfMustCopy, SHGNLI uFlags);
			
			#endregion
			
			#region "       IL functions"
			[DllImport("shell32",EntryPoint="#23", CharSet=CharSet.Auto, SetLastError=true)]
			public static extern bool ILIsParent(IntPtr pidlParent, IntPtr pidlBelow, bool fImmediate);
			
			[DllImport("shell32",EntryPoint="#21", CharSet=CharSet.Auto, SetLastError=true)]
			...
			//{
				...
				//{
					...
					[ComImport(), Guid("00000000-0000-0000-C000-000000000046"), InterfaceType(ComInterfaceType.InterfaceIsIUnknown)]public interface IUnknown
					{
						
						[PreserveSig()] int QueryInterface(ref Guid riid, ref IntPtr pVoid);
					...
					[ComImport(), Guid("00000002-0000-0000-C000-000000000046"), InterfaceType(ComInterfaceType.InterfaceIsIUnknown)]public interface IMalloc
					{
						...
						[PreserveSig()] IntPtr Alloc(int cb); // Size, in bytes, of the memory block to be allocated.
						...
						[PreserveSig()] void Free(IntPtr pv); // Pointer to the memory block to be freed.
						...
						[PreserveSig()] int GetSize(IntPtr pv); // Pointer to the memory block for which the size is requested.
						...
						[PreserveSig()] short DidAlloc(IntPtr pv); // Pointer to the memory block
					...
					{
						[PreserveSig()] int ParseDisplayName(int hwndOwner, IntPtr pbcReserved, [ MarshalAs(UnmanagedType.LPWStr) ]string lpszDisplayName, ref int pchEaten, ref IntPtr ppidl, ref int pdwAttributes);
						
						[PreserveSig()] int EnumObjects(int hwndOwner, [ MarshalAs(UnmanagedType.U4) ]SHCONTF grfFlags, ref IEnumIDList ppenumIDList);
						
						[PreserveSig()] int BindToObject(IntPtr pidl, IntPtr pbcReserved, ref Guid riid, ref IShellFolder ppvOut);
						
						[PreserveSig()] int BindToStorage(IntPtr pidl, IntPtr pbcReserved, ref Guid riid, IntPtr ppvObj);
						
						[PreserveSig()] int CompareIDs(IntPtr lParam, IntPtr pidl1, IntPtr pidl2);
						
						[PreserveSig()] int CreateViewObject(IntPtr hwndOwner, ref Guid riid, ref IUnknown ppvOut);
						
						[PreserveSig()] int GetAttributesOf(int cidl, [ MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 0) ]IntPtr[] apidl, ref SFGAO rgfInOut);
						
						[PreserveSig()] int GetUIObjectOf(IntPtr hwndOwner, int cidl, [ MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 0) ]IntPtr[] apidl, ref Guid riid, ref int prgfInOut, ref IUnknown ppvOut);
						//ByRef ppvOut As IDropTarget) As Integer
						
						[PreserveSig()] int GetDisplayNameOf(IntPtr pidl, [ MarshalAs(UnmanagedType.U4) ]SHGDN uFlags, IntPtr lpName);
						
						[PreserveSig()] int SetNameOf(int hwndOwner, IntPtr pidl, [ MarshalAs(UnmanagedType.LPWStr) ]string lpszName, [ MarshalAs(UnmanagedType.U4) ]SHCONTF uFlags, ref IntPtr ppidlOut);
					}
					#endregion
					...
					{
						[PreserveSig()] int GetNext(int celt, ref IntPtr rgelt, ref int pceltFetched);
						
						[PreserveSig()] int Skip(int celt);
						
						[PreserveSig()] int Reset();
						
						[PreserveSig()] int Clone(ref IEnumIDList ppenum);
					}
					...
					[ComImportAttribute(), InterfaceTypeAttribute(ComInterfaceType.InterfaceIsIUnknown), Guid("0000010B-0000-0000-C000-000000000046")]public interface IPersistFile
					{
                        ...
                        int GetCurFile([In(), Out(), MarshalAs(UnmanagedType.LPWStr)]ref string ppszFileName);
					}
					#endregion
					...
					[ComImportAttribute(), InterfaceType(ComInterfaceType.InterfaceIsIUnknown), Guid("000214EE-0000-0000-C000-000000000046")]public interface IShellLink
					{
                        ...
                        int GetPath([In(), Out(), MarshalAs(UnmanagedType.LPStr)]StringBuilder pszFile, int cchMaxPath, [In(), Out()]ref WIN32_FIND_DATA pfd, SLGP fFlags);
						...
						int GetIconLocation([ Out(), MarshalAs(UnmanagedType.LPStr) ]StringBuilder pszIconPath, int cchIconPath, ref int piIcon);
					...
					[StructLayout(LayoutKind.Sequential, CharSet = CharSet.Auto)]public struct FORMATETC
					{
						public CF cfFormat;
						public IntPtr ptd;
					...
					[StructLayout(LayoutKind.Sequential)]public struct STGMEDIUM
					{
						public int tymed;
					...
					[StructLayout(LayoutKind.Sequential)]public struct DROPFILES
					{
						public int pFiles;
						public Point pt;
						public bool fNC;
					...
					[ComImportAttribute(), InterfaceTypeAttribute(ComInterfaceType.InterfaceIsIUnknown), Guid("00000103-0000-0000-C000-000000000046")]public interface IEnumFORMATETC
					{
						
						[PreserveSig()] int GetNext(int celt, ref FORMATETC rgelt, ref int pceltFetched);
						
						[PreserveSig()] int Skip(int celt);
						
						[PreserveSig()] int Reset();
						
						[PreserveSig()] int Clone(ref IEnumFORMATETC ppenum);
					}
					...
					//{
						...
						[ComImportAttribute(), InterfaceTypeAttribute(ComInterfaceType.InterfaceIsIUnknown), Guid("0000010e-0000-0000-C000-000000000046")]public interface IDataObject
						{
							
							[PreserveSig()] int GetData(ref FORMATETC pformatetcIn, ref STGMEDIUM pmedium);
							[PreserveSig()] int GetDataHere(ref FORMATETC pformatetcIn, ref STGMEDIUM pmedium);
							[PreserveSig()] int QueryGetData(ref FORMATETC pformatetc);
							
							[PreserveSig()] int GetCanonicalFormatEtc(FORMATETC pformatetc, ref FORMATETC pformatetcout);
							[PreserveSig()] int SetData(ref FORMATETC pformatetcIn, ref STGMEDIUM pmedium, bool frelease);
							[PreserveSig()] int EnumFormatEtc(int dwDirection, ref IEnumFORMATETC ppenumFormatEtc);
							[PreserveSig()] int DAdvise(ref FORMATETC pformatetc, ADVF advf, IAdviseSink pAdvSink, ref int pdwConnection);
							
							[PreserveSig()] int DUnadvise(int dwConnection);
							
							[PreserveSig()] int EnumDAdvise(ref object ppenumAdvise);
						}
						...
						[ComImportAttribute(), InterfaceTypeAttribute(ComInterfaceType.InterfaceIsIUnknown), Guid("0000010f-0000-0000-C000-000000000046")]public interface IAdviseSink
						{
						...
						//{
							...
							[ComImportAttribute(), InterfaceType(ComInterfaceType.InterfaceIsIUnknown), Guid("00000122-0000-0000-C000-000000000046")]public interface IDropTarget
							{
								[PreserveSig()] int DragEnter(IntPtr pDataObj, int grfKeyState, Point pt, ref int pdwEffect);
								
								[PreserveSig()] int DragOver(int grfKeyState, Point pt, ref int pdwEffect);
								
								[PreserveSig()] int DragLeave();
								
								[PreserveSig()] int DragDrop(IntPtr pDataObj, int grfKeyState, Point pt, ref int pdwEffect);
							}
							#endregion
